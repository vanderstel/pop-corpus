...
# If the offtime of the last note was not set by a ".", then it has not been set; set it to the last barline.
if($n_off_mdiv[$n-1] == -1) {
    $noteoff[$n-1] = $m;
}

if($melisma_marker != 0) {
    printf("Error: Song ends with an open melisma (%d)\n", $melisma_marker);
    exit;
}
...
sub read_pitch_string {
    ...
    # melisma_marker: if 1, the next current starts a melisma; if -1, it continues a melisma; if 0, it's not in a melisma. Then $melisma[] is set for each
    # note similarly; 1 if it starts a melisma, -1 if it continues it, 0 if it's not in one. Note that the marker can be set even when "(" precedes a dot or ")" follows
    # one. If "(" precedes a dot, the next note that's encountered will be treated as starting the melisma. If ")" follows a dot, the marker is reset to 0 and
    # the next note encountered is assumed to not be in a melisma.

    for($c=0; $c < @chars; $c++) {

	if($chars[$c] eq "(") {
	    if($melisma_marker != 0) {
		printf("Error: You can't start a melisma within a melisma\n");
		exit;
	    }
	    $melisma_marker = 1;
	    next;
	...
	if($chars[$c] eq ")") {
	    if($melisma_marker == 0) {
		printf("Error: You can't end a melisma when none is in progress\n");
		exit;
	    }
	    if($melisma_marker == 1) {
		printf("Error: You can't have a melisma that's only one note long\n");
		exit;
	    }
	    $melisma_marker = 0;
	    next;
        ...
        # If you get a "." or a "~": If it's a ".", check to see whether the offset of the _previous_ note has been
        # set yet. If not, set it to the current mdiv. Then increment mdiv.
	if($chars[$c] eq "." || $chars[$c] eq "~") {
	    if($chars[$c] eq ".") {
		if($n > 0) {
		    if($n_off_mdiv[$n-1] == -1) {
			$n_off_mdiv[$n-1] = $mdiv;
	...
	if($melisma_marker == 1) {    # It's the first note of a melisma
	    $melisma_marker = -1;     # Set melisma_marker to -1 for the next note
	    $melisma[$n] = 1;
	}
	elsif($melisma_marker == -1) {    # It's a non-initial note of a melisma
	    $melisma[$n] = -1;
	}
	else {
	    $melisma[$n] = 0;
...
sub set_met_positions {
    ...
    # Set ontimes for the range of notes ($mfirst, n-1)
    for($n2=$mfirst; $n2<$n; $n2++) {
	$notetime[$n2] = $m + ($nmdiv[$n2] / $mdiv);
	#printf("%.3f (%d,%d)\n", $notetime[$n2], $chrom[$n2], $pitch[$n2]);
    }

    # Set offtimes for the range of notes (mfirst-1, n-1)
    for($n2=$mfirst-1; $n2<$n; $n2++) {
	if($n2 < 0) {
	    next;
	}
	# If it's the last note of the previous measure and its noteoff has already been set, skip it
	if($n2 == $mfirst-1 && $noteoff[$n2] > 0) {
	    next;
	}
	# if the $n_off_mdiv[] for the note has been set (triggered by a "."), set its offtime to that; otherwise set it to the ontime of the following note
	# (unless it's note n-1)
	if($n_off_mdiv[$n2] != -1) {
	    $noteoff[$n2] = $m + ($n_off_mdiv[$n2] / $mdiv);

	}
	elsif($n2<$n-1) {
	    $noteoff[$n2] = $notetime[$n2+1];
	}
	# This will also handle the case where the current measure has no symbols and the offset of the last note of the previous measure has not been set. In that case,
	# $n_off_mdiv[] has not been set for that note, and n2 = n-1, so no offset will be set.
    }
...
sub check_rhythm {
	...
	if(!($mdiv == 2 || $mdiv == 4 || $mdiv == 8 || $mdiv == 16 || $mdiv == 32 || $mdiv == 6 || $mdiv == 12 || $mdiv == 24 || $mdiv == 48)) {
	    printf("Error: %s\n", $prev_ws);
	    exit();
	}
    }
...
if($v == 3 && $numnotes > 0) {
    ...
    # Print out ontimes and pitches (like a note list, but without offtimes) (skip non-initial melisma notes!)
